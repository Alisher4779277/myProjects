...
namespace LicenseHelper
{
    public static class ModifyInMemory
    {
        public static void ActivateMemoryPatching()
        {
            ...
            {
                if (assembly.FullName.StartsWith("Aspose."))
            ...
            foreach (Type type in arrType)
            {
                ...
                foreach (MethodInfo info in arrMInfo)
                {
                    ...
                    try
                    {
                        ...
                        if ((strMethod.IndexOf("(System.Xml.XmlElement, System.String)") > 0) && (info.ReturnType.ToString() != "System.String"))
                        {
                            miEvaluation = info;
                            ...
                            if (((assembly.FullName.IndexOf("Aspose.Pdf") == -1) && (nCount == 2)) ||
                                 ((assembly.FullName.IndexOf("Aspose.Pdf") != -1) && (nCount == 4)))
                            {
                                isFound = true;
                                break;
                            }
